...
class cartadminActions extends sfActions
{
  ...
  public function executeProductIndex(sfWebRequest $request) { 
    ...
    if ($request->hasParameter('filter') && ($request->getParameter('filter') != "")) {
      ...
      if (count($prodIds) > 0) {
       ...
       //$this->products = Doctrine::getTable('Product')->createQuery('a')->where('active=1')->execute();
     }   
  ...
  public function executeProductUpdateBatch(sfWebRequest $request) {
    ...
    //$products = new Doctrine_Collection("product");
    $output = array();
  ...
  protected function processProductForm(sfWebRequest $request, sfForm $form) {
    ...
    if ($form->isValid())
    {
      ...
      if (count($categories) > 0) {
        foreach ($categories as $i => $category) { $categoryIds[$i] = $category->getId(); }

        if ((!$category->getNode()->isRoot()) && (count($category->getNode()->getAncestors()) > 0)) {
          $ancestors = $category->getNode()->getAncestors();
            
            foreach ($ancestors as $ancestor) {
              if (!in_array($ancestor->getId(), $categoryIds)) { 
                $pc = new ProductCategory();
                $pc->setProductId($product->getId());
                $pc->setCategoryId($ancestor->getId());
